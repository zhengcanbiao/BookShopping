...
<body>
    ...
    <div class="body">
    	<div class="container">
            ...
            会员管理
	<form name="form1" method="post" action="/EShopping/manager/SearchCustomer.action" enctype="multipart/form-data">
	    ...
	    <c:choose>
	  	...
	  	<c:choose>
	  		<c:when test="${item.valid==true }">
	  			<td><a href="#" id="click1" onclick="return show_confirm('/EShopping/manager/ValidateCustomer.action?customerName=${item.getCustomerName()}&valid=0','1')">冻结</a></td>
	  		</c:when>
	  		<c:otherwise>
	  			<td><a href="#" id="click2" onclick="return show_confirm('/EShopping/manager/ValidateCustomer.action?customerName=${item.getCustomerName()}&valid=1','2')">激活</a></td>
	  		</c:otherwise>
	  	</c:choose>
	  	</tr>
  	</c:forEach>
